"""
Auto Lane Change controller — feature ported from sunnypilot.

Modes (controlled by AutoLaneChangeTimer param):
  -1: Off       — blinker does not initiate any lane change
   0: Nudge     — stock behavior, requires steering torque to confirm
   1: Nudgeless — instant change after a 50 ms safety dwell
   2: 0.5 s     — wait 0.5 s with blinker on, then change
   3: 1 s       — wait 1 s with blinker on, then change
   4: 2 s       — wait 2 s
   5: 3 s       — wait 3 s

Companion AutoLaneChangeBsmDelay (bool) extends the wait by 1 s if
blindspot monitor is active during the dwell.
"""
...
ONE_SECOND_DELAY = -1.0  # subtracted from wait timer to add a 1s extension
...
class AutoLaneChangeController:
  ...
  def read_params(self) -> None:
    ...
    raw = self.params.get("AutoLaneChangeTimer", return_default=True)
    self.lane_change_set_timer = int(raw) if raw is not None else AutoLaneChangeMode.NUDGE
